  ...
  actions: {
    ...
    createData({ commit }, data) {
      return new Promise((resolve, reject) => {
        ...
        api.post('mailbox', data).then(result => {
          if (result.state == 'success') {
    ...
    updateData({ state, commit }, data) {
      return new Promise((resolve, reject) => {
        ...
        api.put('mailbox/' + state.rowActive._id, data).then(result => {
          if (result.state == 'success') {
    ...
    deleteData({ state, commit }, id) {
      return new Promise((resolve, reject) => {
        api.delete('mailbox/' + state.rowActive._id).then(result => {
          if (result.state == 'success')
            resolve(result);
          else
            reject(result);
