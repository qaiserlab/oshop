  ...
  actions: {
    ...
    createData({ commit }, data) {
      return new Promise((resolve, reject) => {
        ...
        api.post('product', data).then(result => {
          if (result.state == 'success') {
    ...
    updateData({ state, commit }, data) {
      return new Promise((resolve, reject) => {
        ...
        api.put('product/' + state.rowActive._id, data).then(result => {
          if (result.state == 'success') {
    ...
    deleteData({ state, commit }, id) {
      return new Promise((resolve, reject) => {
        api.delete('product/' + state.rowActive._id).then(result => {
          if (result.state == 'success')
            resolve(result);
          else
            reject(result);
  ...
  getters: {
    ...
    rs(state) {
      return state.rs.map(row => {
        ...
        row.featuredImage_ = localStorage.archivesUrl + '/' + row.featuredImage;
        row.produk = row.featuredImage_;
        ...
        row.sku_Number = row.skuNumber;
        row.nama_Produk = row.productName;
        row.kategori = row.productCategory.title;
