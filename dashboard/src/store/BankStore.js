  ...
  actions: {
    ...
    createData({ commit }, data) {
      return new Promise((resolve, reject) => {
        ...
        api.post('bank', data).then(result => {
          if (result.state == 'success') {
    ...
    updateData({ state, commit }, data) {
      return new Promise((resolve, reject) => {
        ...
        api.put('bank/' + state.rowActive._id, data).then(result => {
          if (result.state == 'success') {
    ...
    deleteData({ state, commit }, id) {
      return new Promise((resolve, reject) => {
        api.delete('bank/' + state.rowActive._id).then(result => {
          if (result.state == 'success')
            resolve(result);
          else
            reject(result);
  ...
  getters: {
    ...
    rs(state) {
      return state.rs.map(row => {
        ...
        row.no_Rekening = row.accountNumber;
        row.nama_Pemilik = row.accountName;
