  ...
  actions: {
    ...
    fetchData({ commit }, data) {
      ...
      return new Promise((resolve, reject) => {
        api.get('report/serial-number-stock', data).then(result => {
    ...
    createData({ commit }, data) {
      return new Promise((resolve, reject) => {
        ...
        api.post('report/serial-number-stock', data).then(result => {
          if (result.state == 'success') {
    ...
    updateData({ state, commit }, data) {
      return new Promise((resolve, reject) => {
        ...
        api.put('serial-number-stock/' + state.rowActive._id, data).then(result => {
          if (result.state == 'success') {
    ...
    deleteData({ state, commit }, id) {
      return new Promise((resolve, reject) => {
        api.delete('serial-number-stock/' + state.rowActive._id).then(result => {
          if (result.state == 'success')
            resolve(result);
          else
            reject(result);
  ...
  getters: {
    ...
    rs(state) {
      ...
      return state.rs.map(row => {
        ...
        row.fullName = row.rowMember.firstName + ' ' + row.rowMember.lastName;
